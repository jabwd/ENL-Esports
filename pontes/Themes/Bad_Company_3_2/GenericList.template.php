...
function template_show_list($list_id = null)
{
	...
	if (empty($cur_list['rows']) && !empty($cur_list['no_items_label']))
		echo '
				<tr>
					<td class="windowbg first_td" colspan="', $cur_list['num_columns'], '" align="', !empty($cur_list['no_items_align']) ? $cur_list['no_items_align'] : 'center', '"><div class="padding">', $cur_list['no_items_label'], '</div></td>
				</tr>';
	...
	elseif (!empty($cur_list['rows']))
	{
		...
		foreach ($cur_list['rows'] as $id => $row)
		{
			...
			{
				echo '
					<td', empty($row_data['class']) ? ($no == 0 ? ' class="first_td"' : '') : ' class="' . $row_data['class']. ($no == 0 ? ' first_td' : '').  '"', empty($row_data['style']) ? '' : ' style="' . $row_data['style'] . '"', '>', $row_data['value'], '</td>';
					$no++;
			}
